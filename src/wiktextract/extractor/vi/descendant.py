        ...
        if isinstance(node, WikiNode) and node.kind == NodeKind.LIST:
            for list_node in level_node.find_child(NodeKind.LIST):
                for list_item in list_node.find_child(NodeKind.LIST_ITEM):
                    desc_list.extend(
                        extract_desc_list_item(wxr, list_item, [], [])[0]
                    )
        elif (
            isinstance(node, TemplateNode)
            and node.template_name.lower() == "cjkv"
        ):
            desc_list.extend(extract_cjkv_template(wxr, node))
...
def extract_cjkv_template(
    ...
    for list_item in expanded_node.find_child_recursively(NodeKind.LIST_ITEM):
        desc_list.extend(extract_desc_list_item(wxr, list_item, [], [])[0])
    return desc_list
