...
def parse_language(ctx, config, langnode, language):
    ...
    def parse_part_of_speech(posnode, pos):
        ...
        for node in posnode.children:
            if isinstance(node, WikiNode) and node.kind == NodeKind.LIST:
                lists.append(node)
                break
            elif isinstance(node, WikiNode) and node.kind in (
    ...
    def parse_pronunciation1(node):
        ...
        def parse_pronunciation_template_fn(name, ht):
            ...
            if name == "audio":
                ...
                audios.append(audio)
                return ""
            print("parse_pronunciation_template_fn", name)
            return None

        text = clean_node(config, ctx, node,
                          template_fn=parse_pronunciation_template_fn)
        print("PRONUNCIATION:", repr(text))
        for text in text.split(" * "):  # List items generated by macros
            m = re.match("^\(([^)]*)\)", text)
            if m:
                tagstext = m.group(1)
                text = text[m.end():]
    ...
    def parse_pronunciation(node):
        assert isinstance(node, WikiNode)
        if node.kind in (NodeKind.LIST_ITEM, NodeKind.TEMPLATE):
            parse_pronunciation1(node)
        else:
            for x in node.children:
                if not isinstance(x, WikiNode):
                    continue
                parse_pronunciation(x)
    ...
    # XXX parse_preamble(config, data, pos_sectitle, text, p)
...
def clean_node(config, ctx, node, template_fn=None):
    ...
    if isinstance(node, str):
        ...
        v = " ".join(clean_node(config, ctx, x, template_fn=template_fn)
                     for x in node)
    elif not isinstance(node, WikiNode):
        v = str(node)
    else:
        v = ctx.node_to_text(node, template_fn=template_fn)
        #print("clean_node {} -> {}".format(ctx.node_to_wikitext(node),
        #                                   ctx.node_to_html(node)))
    v = clean_value(config, v)
    return v

# XXX Latin {{la-noun|animal<3>}} gives error:
# animal: No gender explicitly specified in headword template using g=, and can't infer gender from lemma spec at ['animal', 'la-noun', '#invoke']
