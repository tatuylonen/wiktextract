...
def parse_old_quotation(config, data, node, index):
    """There are two types of quotes: inline and template-based.
    The first one follows a very natural format, while the second one varies
    wildly: despite there are some basic templates available (quote-book,
    quote-web etc.) the dump also contains a lot of spurious templates being
    autogenerated from Google Books and similar resources of the form
    {RQ: id|param1=val1|param2=val2|...}
    This function only tries to parse the first one.
    TODO: add support for quote templates.
    TODO: parse the dump to extract such autogenerated templates.
    """
    assert isinstance(config, WiktionaryConfig)
    assert isinstance(data, dict)
        ...
        if re.match(r"(\\#)+\\\*", subnode.pattern):
            for index2, item in enumerate(subnode.items):
                ...
                if len(subnode.sublists(index2)) :
                    quotation_text = clean_value(config, subnode.sublists(index2)[0].items[0]).strip()
                    title_regex = r"'''(?P<year>\d+)'''( (?P<month>\w+) (?P<day>\d+))?, ?(?P<author>[^,]+), ?(''|\")(?P<title>.*?)(''|\")(?P<tag>[^,]*)(, ?(?P<publisher>[^,]+?)(, ?(?P<page>.*))?)?(:|$)"
                    match = re.search(title_regex, quotation_title)
                    if match:
                        groups = match.groupdict()
                        quotation = groups.copy()
                        # Strip potential templates within the author name
                        try:
                            author_parsed = wikitextparser.parse(quotation['author'])
                            for t in author_parsed.templates:
                                quotation['author'] = clean_value(
                                    config, t_arg(config, t, 1))
                        finally:
