...
def parse_language(ctx, config, langnode, language, lang_code):
    ...
    def parse_descendants(data, node, is_proto_root_derived_section=False):
        ...
        # of these that I (jmviz) am unaware of...
        ...
        def get_descendants(node):
            ...
            for _, c in node_children(node):
                if (c.kind == NodeKind.TEMPLATE and c.args
                    ...
                    # e.g. the use of {{CJKV}} in Chinese entries). This allows
                    # us to at least record this template with its args for
                    # postprocessing, even if we don't recover the generated
                    # list structure. 
