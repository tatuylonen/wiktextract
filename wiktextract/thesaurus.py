...
def extract_thesaurus_data(wxr):
    ...
    def page_handler(model, title, text):
        if not title.startswith(ns_title_prefix_tuple(wxr, "Thesaurus")):
            return None
    ...
    num_pages = 0
    for word, linkages in wxr.wtp.reprocess(page_handler, autoload=False):
