...
def parse_translation_item_text(ctx, word, data, item, sense, pos_datas,
    ...
    extra_langcodes = set()
    if lang and lang in ctx.LANGUAGES_BY_NAME:
        extra_langcodes.add(ctx.LANGUAGES_BY_NAME[lang])
        # Canonicalize language name (we could have gotten it via
        # alias or other_names)
        lang = ctx.LANGUAGES_BY_CODE[ctx.LANGUAGES_BY_NAME[lang]][0]
        assert lang
    ...
    if m:
        ...
        elif lang + " " + sublang in ctx.LANGUAGES_BY_NAME:
            lang = lang + " " + sublang
        elif sublang + " " + lang in ctx.LANGUAGES_BY_NAME:
            lang = sublang + " " + lang  # E.g., Ancient Egyptian
        elif sublang in ctx.LANGUAGES_BY_NAME:
            lang = sublang
        ...
        parts = item.split()
        if len(parts) > 1 and parts[0] in ctx.LANGUAGES_BY_NAME:
            lang = parts[0]
            item = " ".join(parts[1:])
        else:
    ...
    if langcode is None:
        if lang in ctx.LANGUAGES_BY_NAME:
            langcode = ctx.LANGUAGES_BY_NAME[lang]
